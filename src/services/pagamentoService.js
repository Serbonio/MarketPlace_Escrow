...
const { verificarPagamento } = require('../controllers/pagamentoController');
...
//   const merchantTransactionId = `${method}${orderId}${Date.now().toString().slice(-4)}`;
  ...
  if (method === 'REF') {
    ...
    result = await createGPOCharge({ amount, phoneNumber: phone, orderId, description });
  }
